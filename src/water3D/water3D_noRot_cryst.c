...
int getDirToProt(t_voxelProp ***p,rvec *refCrd,int nProt,int *ng,t_vecR gridOri,t_vecR dgrid) {
	int i,j,k,l;
	t_vecR pt;
	real min,cur,tmp;
	int idx;
	...
	for(i=0;i<ng[0];i++) {
		for(j=0;j<ng[1];j++) {
			for(k=0;k<ng[2];k++) {
				pt.x=gridOri.x+(i+0.5)*dgrid.x;
				pt.y=gridOri.y+(j+0.5)*dgrid.y;
				pt.z=gridOri.z+(k+0.5)*dgrid.z;
				idx=0;
				min=999999.9;
				for(l=0;l<nProt;l++) {
					tmp=pt.x-refCrd[l][0];
...
int gridPos(t_vecR crd,t_vecR ori,t_vecR dgrid,int *pos,t_box box,int *ng) {
	t_vecR link;
	int check;
	...
	if(link.x<0.0 || link.y<0.0 || link.z<0.0) {
		...
		pos[0]=(int)floor(link.x/dgrid.x);
		if(pos[0]>=ng[0]) {
			check=0; /*outside of grid volume*/
		} else {
			pos[1]=(int)floor(link.y/dgrid.y);
			if(pos[1]>=ng[1]) {
				check=0; /*outside of grid volume*/
			} else {
				pos[2]=(int)floor(link.z/dgrid.z);
				if(pos[2]>=ng[2]) {
					check=0; /*outside of grid volume*/
				} else {
...
int getInput(char *fnCOM,char *fnTop,char *fnCrd,char *fnVel,char *fnJob,
		...
		int *nDelay,int *ng,t_vecR *dgrid,real *radSq,char *fnOut,
                int needVelo) {
        FILE *io;
        char buffer[300];
        int format;
...
/*MODULAR*/
	...
	sprintf(volTitle,"CUBE file of average water number density (nm^-3)",delayTime);
        ...
        for(i=0;i<ng[0];i++) { for(j=0;j<ng[1];j++) { for(k=0;k<ng[2];k++) {
                volData[i][j][k]=vox[i][j][k].averMol/(dgrid.x*dgrid.y*dgrid.z*0.001);
        }}}
	...
	sprintf(fnVol,"%s_water3D-numDens.dat",fnOut,tstep,delayTime);
        ...
        for(i=0;i<ng[0];i++) { for(j=0;j<ng[1];j++) { for(k=0;k<ng[2];k++) {
			...
			gridOri.x+(i+0.5)*dgrid.x,gridOri.y+(j+0.5)*dgrid.y,gridOri.z+(k+0.5)*dgrid.z,
			vox[i][j][k].averMol/(dgrid.x*dgrid.y*dgrid.z*0.001));
        }}}
        fclose(out);
...
/*	sprintf(fnVol,"%s_water3D-COMmsd_dt-%.3fps_maxt-%.3fps.dat",fnOut,tstep,delayTime);
	...
	for(i=0;i<ng[0];i++) { for(j=0;j<ng[1];j++) { for(k=0;k<ng[2];k++) {
		fprintf(out,"# %9.5f %9.5f %9.5f dt= %9.5f n= %d\n",
			gridOri.x+(i+0.5)*dgrid.x,gridOri.y+(j+0.5)*dgrid.y,gridOri.z+(k+0.5)*dgrid.z,tstep,vox[i][j][k].nMol);
		for(l=0;l<nDelay;l++) {
...
*/
	...
	sprintf(fnVol,"%s_water3D-COMvel-VDOS_df-%.3fwn_maxf-%.3fwn.dat",fnOut,fstep,nDelay*fstep);
        ...
        for(i=0;i<ng[0];i++) { for(j=0;j<ng[1];j++) { for(k=0;k<ng[2];k++) {
			...
			gridOri.x+(i+0.5)*dgrid.x,gridOri.y+(j+0.5)*dgrid.y,gridOri.z+(k+0.5)*dgrid.z,fstep,vox[i][j][k].nMol);
                for(l=0;l<nDelay;l++) {
                        fprintf(out," %10.5e",vox[i][j][k].vCOMcorr[l]);
                }
                fprintf(out,"\n");
