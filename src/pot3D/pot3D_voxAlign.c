...
int makeLJcombParameters(t_atom *atoms,int nAtoms,int *types,double ***sigSqMat,double ***epsMat) {
        ...
        for(i=0;i<nTypes;i++) {
                for(j=0;j<nTypes;j++) {
                        sigSqMat[0][i][j]=(sigList[i]+sigList[j])/2.0;
                        sigSqMat[0][i][j]*=sigSqMat[0][i][j];
...
int getInput(char *fnCOM,char *fnTop,char *fnCrd,char *fnVel,char *fnJob,
		...
		int *nonSoluteGrp,int *rotAlign,int *wrap,int *ng,real *dgrid,real *cutOffPotSq,
		real *cutOffList,int *LJ,int *ES,int *PBC,char *fnOut,t_vecR *otherCenter,
                int needVelo) {
        FILE *io;
        char buffer[300];
        int format;
	...
	getLineFromCOM(io,buffer,300);
        sscanf(buffer,"%f",&tmp);
        printf("%2d -> read %20s : %f\n",i,"otherCenterX",tmp);i++;
        otherCenter[0].x=tmp;

        getLineFromCOM(io,buffer,300);
        sscanf(buffer,"%f",&tmp);
        printf("%2d -> read %20s : %f\n",i,"otherCenterY",tmp);i++;
        otherCenter[0].y=tmp;

        getLineFromCOM(io,buffer,300);
        sscanf(buffer,"%f",&tmp);
        printf("%2d -> read %20s : %f\n",i,"otherCenterZ",tmp);i++;
        otherCenter[0].z=tmp;
...
int main(int argc, char *argv[])
{
	...
	t_vecR gridOri,otherCenter;
        t_vecR link,gridOri2;
	t_voxelProp *vox;
	t_list *voxMols;
	int ng3,voxIdx;
	int pos[3];
...
/*PARTIALLY MODULAR*/
	...
	getInput(fnCOM,fnTop,fnCrd,fnVel,fnJob,
                ...
                &cutOffList,&LJ,&ES,&PBC,fnOut,&otherCenter,
                needVelo);
...
/*MODULAR */
	...
	link.x=gridOri.x-otherCenter.x;
        link.y=gridOri.y-otherCenter.y;
        link.z=gridOri.z-otherCenter.z;
        pos[0]=(int)floor(link.x/dgrid);
        pos[1]=(int)floor(link.y/dgrid);
        pos[2]=(int)floor(link.z/dgrid);
        gridOri.x=otherCenter.x+pos[0]*dgrid;
        gridOri.y=otherCenter.y+pos[1]*dgrid;
        gridOri.z=otherCenter.z+pos[2]*dgrid;
        gridOri2.x=gridOri.x-otherCenter.x;
        gridOri2.y=gridOri.y-otherCenter.y;
        gridOri2.z=gridOri.z-otherCenter.z;
...
/*MODULAR*/
			...
			for(j=0;j<grps[solventGrp].nMols;j++) {
				...
				if(gridPos(mols[molIdx1].COM,gridOri,dgrid,dgrid,dgrid,pos,box,ng)==1) {
                                        voxIdx=pos[2]+ng[2]*(pos[1]+ng[1]*pos[0]);
					voxMols[voxIdx].idx[voxMols[voxIdx].n]=molIdx1;
					voxMols[voxIdx].n++;
					if(voxMols[voxIdx].n>100) fatal("more than 100 molecules in one voxel\ncurrently hard coded limit\n");
					vox[voxIdx].nMol++;
				}
			}
...
/*MODULAR*/
	...
	l=0;
        for(i=0;i<ng[0];i++) { for(j=0;j<ng[1];j++) { for(k=0;k<ng[2];k++) {
                volData[i][j][k]=vox[l].averMol/(dgrid*dgrid*dgrid*0.001);
		l++;
        }}}
        printVolumeData(fnVol,volTitle,gridOri2,voxel,ng[0],ng[1],ng[2],volData);
