...
int computePot(t_atom *atoms,t_mol *mols,t_box box,int molIdx1,int molIdx2,real cutOffListSq,real cutOffPotSq,
        int LJ,int ES,int PBC,double *U,double **sigSqMat,double **epsMat,int *types,int nAtomsPerResMax,int unscale) {
        real rSq,r;
        int i,l,m;
        int inRange;
	...
	double actualUnscale;
        ...
        if(molIdx1!=molIdx2) {
                ...
                if(inRange==2) {
					...
					if(unscale!=0) {
						if(strncmp(atoms[atomIdx1].atomName,"H",1)==0 && strncmp(atoms[atomIdx2].atomName,"H",1)==0) 
							actualUnscale=2.17391;
						else if(strncmp(atoms[atomIdx1].atomName,"O",1)==0 && strncmp(atoms[atomIdx2].atomName,"H",1)==0)
							actualUnscale=1.47442;
						else actualUnscale=1.0;
					} else {
						actualUnscale=1.0;
					}	
                                        ...
                                        if(LJ==1) U[0]+=
                                          4.0*eps*(sigDivR6*sigDivR6-sigDivR6)/actualUnscale;
