...
int computePot(t_atom *atoms,t_mol *mols,t_box box,int molIdx1,int molIdx2,real cutOffListSq,real cutOffPotSq,
	int LJ,int ES,int PBC,double *U,double **sigSqMat,double **epsMat,int *types,int nAtomsPerResMax,double unscale) {
	real rSq,r;
	int i,l,m;
	int inRange;
		...
		if(inRange==2) {
			for(l=0;l<mols[molIdx1].nAtoms;l++) {
				...
				for(m=0;m<mols[molIdx2].nAtoms;m++) {
					...
					if(LJ==1) U[0]+=
					  4.0*eps*(sigDivR6*sigDivR6-sigDivR6)/unscale;
