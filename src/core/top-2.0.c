...
int getAtomLineFromTOP(char *fn,FILE *in,char *name,real *mass,real *charge,real *pol,real *sig,real *eps,int *lineCnt)
{
	char buffer[300];
	char err[400];
	float dumMass,dumCharge,dumPol,dumSig,dumEps;
	
	getLineFromTOP(in,buffer,lineCnt);
        if(sscanf(buffer,"%s %f %f %f %f %f",name,&dumMass,&dumCharge,&dumPol,&dumSig,&dumEps)!=6) {
...
int cpyMolTop(t_mol src,t_mol *dest,t_atom *atoms,int copy)
{
	...
	for(i=0;i<src.nAtoms;i++)
	{
		...
		atoms[dest[0].atoms[i]].sigmaLJ=atoms[src.atoms[i]].sigmaLJ;
		atoms[dest[0].atoms[i]].epsLJ=atoms[src.atoms[i]].epsLJ;
	}
	for(i=0;i<src.nSites;i++)
	{
		...
		dest[0].sites[i].sigmaLJ=src.sites[i].sigmaLJ;
		dest[0].sites[i].epsLJ=src.sites[i].epsLJ;
	}
...
int readTOP(char *fn,t_atom **atoms,int *nAtoms,t_vecR **siteCrds,int *nSites,t_mol **mols,int *nMols)
{
	...
	for(i=0;i<nMolTypes;i++) {
		...
		for(j=0;j<mols[0][molCnt].nAtoms;j++) {
			...
			getAtomLineFromTOP(fn,in,atoms[0][atomCnt].atomName,&atoms[0][atomCnt].mass,&atoms[0][atomCnt].charge,&atoms[0][atomCnt].pol,&atoms[0][atomCnt].sigmaLJ,&atoms[0][atomCnt].epsLJ,&lineCnt);
		...
		{
			getAtomLineFromTOP(fn,in,mols[0][molCnt].sites[j].siteName,&mols[0][molCnt].sites[j].mass,&mols[0][molCnt].sites[j].charge,&mols[0][molCnt].sites[j].pol,&mols[0][molCnt].sites[j].sigmaLJ,&mols[0][molCnt].sites[j].epsLJ,&lineCnt);
