...
#now we count all run-NPT+posres_x directories and check their content for completeness
i=1
...
cd run-NPT+posres_${i}
    for file in ${cubes[@]}
    do
        if [ ! -f average/${prefix}_${file}.cube ]; then
            echo "-file run-NPT+posres_${i}/average/${prefix}_${file}.cube not found"
    ...
    for file in ${data[@]}
    do
        if [ ! -f average/${prefix}_${file}.dat ]; then
            echo "-file run-NPT+posres_${i}/average/${prefix}_${file}.dat not found"
            echo "-step 3 has not been completed yet or caused an error"
            echo "-exiting"
            exit
        fi
    done
    echo "-directory run-NPT+posres_${i} contains averaged (level 1) 3D-2PT data set"
cd ..
((i+= 1))
done
if [ ${i} -eq 1 ]; then
echo "-no 3D-2PT data found for level 2 averaging"
echo "-exiting"
exit
fi
iMax=`expr $i - 1`
echo "-found ${iMax} directories with averaged (level 1) 3D-2PT data sets"

#We extract the dimensions of the analysis grid directly from one of the output files
grid=`head -n 6 run-NPT+posres_1/average/${prefix}_water3D-numDens.cube | tail -n 3 | awk '{printf("%d ",$1);};'`
...
echo "../run-NPT+posres_${i}/average/${prefix}_${avCubes[cubeIdx]}.cube"
if [ $i -eq ${iMax} ]; then
...
echo "../run-NPT+posres_${i}/average/${prefix}_${w1avCubes[cubeIdx]}.cube"
echo "../run-NPT+posres_${i}/average/${prefix}_pot3D-numDens.cube"
if [ $i -eq ${iMax} ]; then
...
echo "../run-NPT+posres_${i}/average/${prefix}_${w2avCubes[cubeIdx]}.cube"
echo "../run-NPT+posres_${i}/average/${prefix}_water3D-numDens.cube"
if [ $i -eq ${iMax} ]; then
...
echo "../run-NPT+posres_${i}/average/${prefix}_${data[dataIdx]}.dat"
if [ $i -eq ${iMax} ]; then
echo "200"
echo "${grid}"
